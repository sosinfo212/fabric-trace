...
interface SaleOrderComboboxProps {
  value: string;
  onValueChange: (value: string, designation: string | null) => void;
  placeholder?: string;
  disabled?: boolean;
  className?: string;
}

export function SaleOrderCombobox({
  value,
  onValueChange,
  ...
  const [options, setOptions] = useState<SaleOrderOption[]>([]);
  const [loading, setLoading] = useState(false);

  const fetchOptions = useCallback(async (q: string) => {
    setLoading(true);
    try {
      const res = await transfertApi.getSaleOrders(q || undefined);
      setOptions(res.data);
    } catch {
      setOptions([]);
    } finally {
  ...
  useEffect(() => {
    if (open && options.length === 0 && !search) fetchOptions('');
  }, [open, fetchOptions, search, options.length]);
            ...
            <CommandGroup>
              {options.map((opt) => (
                <CommandItem
                  ...
                  value={opt.id}
                  onSelect={() => {
                    onValueChange(opt.id, opt.designation);
                    setOpen(false);
                  }}
                >
